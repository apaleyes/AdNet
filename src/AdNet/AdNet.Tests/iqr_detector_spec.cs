...
namespace AdNet.Tests
{
    class iqr_detector_spec : nspec
    {
        ...
        void detects_single_anomaly_in_data()
        {
            act = () =>
            {
                detector = new IqrDetector<double, int>(20, 80);
                anomalies = detector.DetectAnomalies(testSeries);
            };

            context["given data with anomaly in the beginning"] = () =>
            {
                before = () =>
                {
            ...
            context["when all points are within the thresholds"] = () =>
            {
                before = () =>
                {
                    ...
                    detector = new IqrDetector<double, int>(2.0, 2.0);
                };
        ...
        public void requires_both_percentile_ranks()
        {
            context["when no percentile ranks are given"] = () =>
            {
                it["throws ArgumentException"] =
                    expect<ArgumentException>(() => new IqrDetector<double, int>(null, null));
            };
        }
